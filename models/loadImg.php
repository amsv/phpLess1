...
function changeImage($h, $w, $src, $newsrc, $type) {
 ...
 if (isset($_POST['send'])) {
    if ($_FILES['userfile']['error']) {
      ...
      $message = 'Файл слишком большой';
        ...
        $_FILES['userfile']['type'] == 'image/gif'
          ...
          if (copy($_FILES['userfile']['tmp_name'], DIR_PHOTO_BIG.translit($_FILES['userfile']['name']))) {
            $path = DIR_PHOTO_SMALL.translit($_FILES['userfile']['name']);
  ...
  $images = array_slice(scandir(DIR_PHOTO_SMALL), 2);
/*
if (move_uploaded_file($_FILES['userfile']['tmp_name'], DIR_PHOTO_BIG.translit($_FILES['userfile']['name']))) {
    $path = DIR_PHOTO_SMALL.translit($_FILES['userfile']['name']);
    $type = explode('/', $_FILES['userfile']['type'])[1];
    changeImage(150, 150, $_FILES['userfile']['tmp_name'], $path, $type);
} else {$message = 'Ошибка загрузки файла!';}
...
/*
if(move_uploaded_file($_FILES['photo']['tmp_name'],DIR_PHOTO_BIG)){
    //header('Location: /?inf='.$_FILES['photo']['name'].' успешно загружен на сервер!');
}
else{
